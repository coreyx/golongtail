...
func copyOneVersion(
	...
	maxChunksPerBlock uint32,
	retainPermissions bool,
	minBlockUsagePercent uint32) error {
	...
	hashIdentifier := sourceVersionIndex.GetHashIdentifier()
	sourceTargetChunkSize := sourceVersionIndex.GetTargetChunkSize()

	targetIndexReader := asyncVersionIndexReader{}
	targetIndexReader.read(targetPath,
		nil,
		sourceTargetChunkSize,
		noCompressionType,
		hashIdentifier,
		pathFilter,
		fs,
	...
	originalTargetVersionIndex, hash, _, err := targetIndexReader.get()
	if err != nil {
		return nil
	}
	defer originalTargetVersionIndex.Dispose()

	versionDiff, errno := longtaillib.CreateVersionDiff(
		hash,
		originalTargetVersionIndex,
		sourceVersionIndex)
	if errno != 0 {
		return errors.Wrapf(longtaillib.ErrnoToError(errno, longtaillib.ErrEIO), "cloneStore: longtaillib.CreateVersionDiff() failed")
	}
	defer versionDiff.Dispose()
	...
	chunkHashes, errno := longtaillib.GetRequiredChunkHashes(
		...
		versionDiff)
	if errno != 0 {
		return errors.Wrapf(longtaillib.ErrnoToError(errno, longtaillib.ErrEIO), "cloneStore: longtaillib.GetRequiredChunkHashes() failed")
	}

	existingStoreIndex, errno := getExistingStoreIndexSync(sourceStore, chunkHashes, 0)
	chunkHashes = nil
	if errno != 0 {
		return errors.Wrapf(longtaillib.ErrnoToError(errno, longtaillib.ErrEIO), "cloneStore: getExistingStoreIndexSync() failed")
	}
	defer existingStoreIndex.Dispose()
	...
	if errno != 0 {
		...
		if err != nil {
			return errors.Wrapf(err, "cloneStore: zip.OpenReader() failed")
		}
	...
	// Re-scan with the desired target chunk size
	targetFolderScanner = asyncFolderScanner{}
	targetFolderScanner.scan(targetPath, pathFilter, fs)
	...
	existingStoreIndex, errno = getExistingStoreIndexSync(targetStore, targetVersionIndex.GetChunkHashes(), minBlockUsagePercent)
	if errno != 0 {
		return errors.Wrapf(longtaillib.ErrnoToError(errno, longtaillib.ErrEIO), "cloneStore: getExistingStoreIndexSync() failed")
	}
	defer existingStoreIndex.Dispose()
	...
	versionMissingStoreIndex, errno := longtaillib.CreateMissingContent(
		...
		maxChunksPerBlock)
	if errno != 0 {
		return errors.Wrapf(longtaillib.ErrnoToError(errno, longtaillib.ErrEIO), "cloneStore: CreateMissingContent() failed")
	}
	defer versionMissingStoreIndex.Dispose()
	...
	if versionMissingStoreIndex.GetBlockCount() > 0 {
		...
		if errno != 0 {
			return errors.Wrapf(longtaillib.ErrnoToError(errno, longtaillib.ErrEIO), "cloneStore: longtaillib.WriteContent() failed")
		}
	}
	...
	if errno != 0 {
		return errors.Wrapf(longtaillib.ErrnoToError(errno, longtaillib.ErrEIO), "cloneStore: targetRemoteStore.Flush failed")
	}
	...
	vbuffer, errno = longtaillib.WriteVersionIndexToBuffer(targetVersionIndex)
	if errno != 0 {
		return errors.Wrapf(longtaillib.ErrnoToError(errno, longtaillib.ErrEIO), "cloneStore: longtaillib.WriteVersionIndexToBuffer() failed")
	}

	err = longtailstorelib.WriteToURI(targetFilePath, vbuffer)
	vbuffer = nil
	if err != nil {
		return errors.Wrapf(longtaillib.ErrnoToError(errno, longtaillib.ErrEIO), "cloneStore: longtailstorelib.WriteToURI() failed")
	}
	...
	if createVersionLocalStoreIndex {
		...
		if errno != 0 {
			return errors.Wrapf(longtaillib.ErrnoToError(errno, longtaillib.ErrEIO), "cloneStore: longtaillib.MergeStoreIndex() failed")
		}
		versionLocalStoreIndexBuffer, errno := longtaillib.WriteStoreIndexToBuffer(versionLocalStoreIndex)
		versionLocalStoreIndex.Dispose()
		if errno != 0 {
			return errors.Wrapf(longtaillib.ErrnoToError(errno, longtaillib.ErrEIO), "cloneStore: longtaillib.WriteStoreIndexToBuffer() failed")
		}
		...
		err = longtailstorelib.WriteToURI(versionLocalStoreIndexPath, versionLocalStoreIndexBuffer)
		versionLocalStoreIndexBuffer = nil
		if err != nil {
			return errors.Wrapf(longtaillib.ErrnoToError(errno, longtaillib.ErrEIO), "cloneStore: longtailstorelib.WriteToURI() failed")
		}
	}
	...
	if targetStoreFlushComplete.err != 0 {
		return errors.Wrapf(longtaillib.ErrnoToError(errno, longtaillib.ErrEIO), "cloneStore: targetStoreFlushComplete.Flush failed")
	}
	sourceStoreFlushComplete.wg.Wait()
	if sourceStoreFlushComplete.err != 0 {
		return errors.Wrapf(longtaillib.ErrnoToError(errno, longtaillib.ErrEIO), "cloneStore: sourceStoreFlushComplete.Flush failed")
	}
	return nil
}
